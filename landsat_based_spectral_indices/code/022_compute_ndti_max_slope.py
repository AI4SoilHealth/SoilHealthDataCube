...
for i in range(start_tile, end_tile):
  ...
  try:
    ...
    raster_files['ndti'] = f'http://192.168.49.30:8333/tmp-eumap-ai4sh/{tile}/skmap_ndti_' + '{dt}.tif'

    rdata = RasterData(raster_files, max_rasters=400, verbose=True
    ...
    rdata = rdata.read(gdal_opts=gdal_opts, n_jobs=96
            ).run(process.FindMinMax(season_size = 6, min_max='min'),
                  group='ndti', outname= '{gr}_glad.landsat.ard2.seasconv.bimonthly.m.{nm}_{pr}_30m_s_{dt}_eu_epsg.3035_v20231218'
            ).drop(group=['ndti']
            ).to_s3(
              host = hosts,
              access_key='iwum9G1fEQ920lYV4ol9',
              secret_key='GMBME3Wsm8S7mBXw3U4CNWurkzWMqGZ0n2rXHggS0',
              path=f"tmp-eumap-ai4sh/test/{tile}",
              secure=False,
              n_jobs=96,
              verbose_cp = False
            ).run(process.SlopeAnalysis(scale_expr = '(data - 125) / 125', scaling = 10000), 
                  group='ndti.min', outname= '{gr}_glad.landsat.ard2.seasconv.yearly.min.m.{nm}_{pr}_30m_s_{dt}_eu_epsg.3035_v20231218'
            ).drop(group=['ndti.min']
            ).to_s3(
              host = hosts,
              access_key='iwum9G1fEQ920lYV4ol9',
