...
for i in range(start_tile, end_tile):
  ...
  try:
    ...
    raster_files['bs'] = f'http://192.168.49.30:8333/tmp-eumap-ai4sh/{tile}/skmap_aggregate.bs.yearly_sum_' + '{dt}.tif'
    raster_files['ndvi'] = f'http://192.168.49.30:8333/tmp-eumap-ai4sh/{tile}/skmap_aggregate.ndvi.yearly_p50_' + '{dt}.tif'

    rdata = RasterData(raster_files, max_rasters=200, verbose=True
    ...
    rdata = rdata.read(gdal_opts=gdal_opts, n_jobs=96,
	    ).run(process.SlopeAnalysis(scale_expr = 'data / 100', scaling = 10000), group='bs', outname= '{gr}_glad.landsat.ard2.seasconv.yearly.m.{nm}_{pr}_30m_s_{dt}_eu_epsg.3035_v20231218'
            ).drop(group=['bs']
            ).run(process.SlopeAnalysis(scale_expr = '(data - 125) / 125', scaling = 10000), group='ndvi', outname= '{gr}_glad.landsat.ard2.seasconv.yearly.m.{nm}_{pr}_30m_s_{dt}_eu_epsg.3035_v20231218'
            ).drop(group=['ndvi']
            ).to_s3(
              host = hosts,
